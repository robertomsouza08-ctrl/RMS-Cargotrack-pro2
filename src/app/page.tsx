  ...
  try {
              ...
              {shipments.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{padding:24, textAlign:'center', color:'#6b7280'}}>
                    Nenhum shipment encontrado. {isAuthed ? 'Crie o primeiro acima!' : 'Entre para criar.'}
                  </td>
                </tr>
              ) : (
                shipments.map((s) => (
    ...
    return (
      <div style={{padding:24, background:'rgba(255,255,255,0.95)', borderRadius:12, border:'2px solid #ef4444'}}>
        <h1 style={{color:'#ef4444', marginTop:0}}>Erro ao carregar página</h1>
        <p><strong>Mensagem:</strong> {error.message}</p>
        <p><strong>Possíveis causas:</strong></p>
        <ul>
          <li>Banco de dados não está acessível</li>
          <li>Variável DATABASE_URL não configurada</li>
          <li>Migrations não foram aplicadas</li>
        </ul>
        <p><strong>Soluções:</strong></p>
        <ol>
          <li>Verifique se o PostgreSQL está rodando no Railway</li>
          <li>Confirme que DATABASE_URL está configurada corretamente</li>
          <li>Rode: <code>npx prisma migrate deploy</code></li>
        </ol>
      </div>
    )
  }
